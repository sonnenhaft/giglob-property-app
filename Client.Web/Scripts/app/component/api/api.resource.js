angular.module('api.resource', [
    ...
    return $resource(currentServer + '/v1/:type/:action', null, {
        save: { method: 'POST' },
        getMyOffers: { method: 'GET', isArray: true }
    });
    ...
    return $resource(currentServer + '/v1/user/register', null, { save: { method: 'POST' } });
}).factory('login', function ( $resource, currentServer ) {
    return $resource(currentServer + '/v1/user/signin', null, { save: { method: 'POST' } });
}).factory('confirm', function ( $resource, currentServer ) {
    return $resource(currentServer + '/v1/user/confirmemail', null, { save: { method: 'POST' } });
}).factory('getProperty', function ( $resource, $rootScope, currentServer ) {
    return $resource(currentServer + '/v1/propertyoffer/get/:id', null, {
        save: { method: 'POST' },
